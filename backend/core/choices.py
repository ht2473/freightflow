...
class NaturalKind(models.TextChoices):
    """Вид природной территории на подложке карты.

    Разделение задаёт не только цвет: водная поверхность — препятствие для
    движения, зелёный массив — территория, застройка которой ограничена.
    Обе величины участвуют в оценке того, где инфраструктура разместиться
    не может.
    """

    WATER = "water", _("Водная поверхность")
    GREEN = "green", _("Зелёный массив")
...
class VerificationState(models.TextChoices):
    """Итог осмотра записи реестра человеком.

    Осмотр не правит сведения источника: разметка OpenStreetMap исправляется
    в самой разметке, иначе правка была бы стёрта следующей же загрузкой,
    а величина потеряла бы прослеживаемость. Осмотр отвечает на другой
    вопрос — соответствует ли запись действительности, и его итог виден
    рядом с записью.
    """

    NONE = "", _("Не проверялась")
    CONFIRMED = "confirmed", _("Подтверждена")
    DISPUTED = "disputed", _("Требует уточнения")
