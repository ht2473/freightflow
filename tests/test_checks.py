...
class TestStaticCacheHeaders:
    ...
    Метод: функциональное тестирование.

    Штатный обработчик статики в runserver перехватывает запросы до цепочки
    middleware и заголовка Cache-Control не выставляет, а браузер в его
    отсутствие кеширует ответ по собственной эвристике. Поэтому статику
    в обоих контурах обслуживает WhiteNoise.
    """
