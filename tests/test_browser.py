...
Набор закрывает разрыв в методике, из-за которого отказ карты оставался
незамеченным. Остальные проверки обращаются к приложению тестовым клиентом
Django: он получает разметку, но не исполняет сценарии и не разбирает
встроенные в страницу данные. Карта же не работала именно на стороне
клиента — разметка отдавалась с кодом 200, а разбор настроек падал.

Здесь страницы открываются управляемым браузером, и любое сообщение об
ошибке в консоли считается отказом.
...
class TestMap:
    ...
    def test_map_initializes(self, live_server, offline_page, console_errors, full_dataset):
        ...
        разметку Leaflet и на нём появились плитки подложки. При прежнем
        отказе контейнер оставался пустым.
        """
    ...
    def test_map_centered_on_moscow(self, live_server, offline_page, full_dataset):
        ...
        Прежний отказ разбирал «55,7522» как два целых числа, и Leaflet
        получал бессмысленный центр. Проверка смотрит на разобранные
        клиентом настройки, а не на разметку.
        """
        offline_page.goto(f"{live_server.url}/map/", wait_until="networkidle")
