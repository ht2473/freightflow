...
Система обслуживается собственным доменом целиком: шрифты, библиотека карты,
тайлы и подложка приходят с неё же. Правило это объявлено политикой
безопасности содержимого, и его соблюдение проверяет браузер — обращение
к чужому домену, попавшее в разметку по недосмотру, не выполнится.

| Заголовок | Кто ставит | Значение |
|---|---|---|
| `Content-Security-Policy` | приложение | `default-src 'self'` и уточнения по видам ресурсов |
| `X-Content-Type-Options` | обратный прокси | `nosniff` |
| `X-Frame-Options` | обратный прокси и приложение | `SAMEORIGIN` / `DENY` |
| `Referrer-Policy` | обратный прокси и приложение | ограничение передачи адреса источника |
| `Strict-Transport-Security` | обратный прокси | год, включая поддомены |

Политику ставит приложение (`core.middleware.ContentSecurityPolicyMiddleware`),
а не обратный прокси: статику раздаёт сам контейнер через WhiteNoise, и
политика должна действовать при любом способе развёртывания — в том числе
без nginx перед приложением.

Прочие заголовки собраны в `deploy/security_headers.conf` и включаются
в конфигурацию nginx трижды: в блоке `server` и в каждом `location`,
объявляющем собственные заголовки. Повторное включение обязательно —
nginx не складывает директивы `add_header` разных уровней, а берёт
ближайший набор целиком, поэтому без него статические файлы отдавались бы
без указания не угадывать тип содержимого.

Послабления в политике объявлены поимённо и вызваны устройством интерфейса:

- `style-src 'unsafe-inline'` — ширина полосы доли и предельная ширина
  столбца задаются атрибутом `style` у элемента;
- `img-src data:` — значки элементов управления картой приходят вложенными
  в таблицу стилей библиотеки;
- `worker-src blob:` — разбор векторных тайлов библиотека карты ведёт
  в фоновом потоке, создаваемом из собственного кода.

Послабления для сценариев нет: встроенных сценариев в разметке не осталось,
установка оформления до первой отрисовки вынесена в отдельный файл.
Отсутствие встроенных сценариев проверяется набором тестов — встроенный
сценарий при объявленной политике не исполнился бы, и раздел перестал бы
работать молча.

---

## 7. Наблюдение за системой
